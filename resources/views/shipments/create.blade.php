                ...
                <div class="card h-100 shadow-sm border-0 quote-card">
                    ...
                    <div class="card-header bg-white border-0 d-flex justify-content-between align-items-center py-3">
                        ...
                        <span class="badge rounded-pill px-3 py-2
                            {{ $shipment->status === 'completed' ? 'bg-success' : 'bg-warning' }}">
                        ...
                        <div class="row mb-3 text-center text-md-start">
                            <div class="col-6">
                                ...
                                <p class="mb-0 fw-bold text-primary">
                                    {{ $locationTypes->firstWhere('code', $shipment->pickup_location)->name
                                        ?? ucfirst($shipment->pickup_location) }}
                                </p>
                            </div>
                            <div class="col-6">
                                <small class="text-muted d-block">Delivery</small>
                                <p class="mb-0 fw-bold text-success">
                                    {{ $locationTypes->firstWhere('code', $shipment->drop_location)->name
                                        ?? ucfirst($shipment->drop_location) }}
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-6">
                                ...
                                @if ($shipment->pickupDetail && $shipment->pickupDetail->city)
                                    <p class="mb-0 fw-semibold">
                                        {{ $shipment->pickupDetail->city }}, {{ $shipment->pickupDetail->state }}
                                    </p>
                                    @if ($shipment->pickupDetail->postal_code)
                                        <small class="text-muted">{{ $shipment->pickupDetail->postal_code }}</small>
                                    @endif
                                @else
                            ...
                            <div class="col-6">
                                ...
                                @if ($shipment->deliveryDetail && $shipment->deliveryDetail->city)
                                    <p class="mb-0 fw-semibold">
                                        {{ $shipment->deliveryDetail->city }}, {{ $shipment->deliveryDetail->state }}
                                    </p>
                                    @if ($shipment->deliveryDetail->postal_code)
                                        <small class="text-muted">{{ $shipment->deliveryDetail->postal_code }}</small>
                                    @endif
                                @else
                                    <span class="text-muted fst-italic">—</span>
                                @endif
                    ...
                    <div class="card-footer bg-transparent border-0 pt-0">
                        @if ($latestResponse && $latestResponse->status === 'success')
                        @php
                        $shipment_id = encrypt($shipment->id);
                        @endphp
                            <a href="{{ route('quotes.show',$shipment_id) }}"
